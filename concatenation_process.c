#include "holberton.h"
/**
 * concat_commands - creates a double ptr array of all your directories
...
int concat_commands(char **commands, char *buffer, char **env)
{
	...
	while (path_dir[i] != NULL)
	{
		if (access(path_dir[i], X_OK) == 0)
			execve(path_dir[i], commands, env);
		i++;
	}
	if (path_dir[i] ==  NULL)
	{
...
/**
 * get_path_dir - creates a double pointer array
 * @commands: commands that you type in the prompt
...
/**
 * get_path_str - gets the environment variable value
 * @env: the enviroment variables
